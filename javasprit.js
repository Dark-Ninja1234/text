...
input.addEventListener("keyup",(event) =>
     {
          ...
          spancontainertag.innerHTML = "";
          const serch = event.target.value.toLowerCase();
          if (serch.length === 0 ) {
             return
          }
           fitertag = shop.filter((item ) => {
          return item.title.toLowerCase().includes(serch);


        });
           const filterone = fitertag.length > 0;
           if (filterone){
            console.log(filterone)
           for (let i = 0; i < fitertag.length; i++) {
           const containerdiv = document.createElement("div");
           containerdiv.classList.add("maincontainer");
           containerdiv.id = fitertag[i].id;

           const titletag = document.createElement("div");
           titletag.classList.add('titletagclass');
           titletag.append( fitertag[i].title);


          const imgtag = document.createElement("img");
          imgtag.src = fitertag[i].image;
          imgtag.classList.add("imgtag");
          containerdiv.append(titletag, imgtag);
          spancontainertag.append(containerdiv);
        }
